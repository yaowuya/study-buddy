<template>
  <view class="page">
    ...
    <view class="appbar" :style="{ paddingTop: Math.max(statusBarHeight, 12) + 'px' }">
      ...
      <text class="appbar-brand">编辑任务</text>
    </view>

    <scroll-view scroll-y class="main" :style="{ paddingTop: (Math.max(statusBarHeight, 12) + 56) + 'px' }">
      <!-- Page Title -->
      <view class="page-header">
        <text class="page-title">编辑任务</text>
        <text class="page-subtitle">修改任务内容和听写设置。</text>
      </view>
      ...
      <view class="submit-btn" @tap="handleSubmit">
        <text class="material-symbols-outlined submit-icon">save</text>
...
.delete-btn {
  ...
  padding: $spacing-md; border-radius: 20px;
  border-bottom: 6px solid #7d0f0f;
  font-family: $font-family; font-size: 18px; font-weight: 600;
  margin-bottom: $spacing-lg;
  &:active { border-bottom-width: 0; transform: translateY(6px); }
}
.delete-icon { font-size: 22px; color: #93000a; }
.delete-text { color: #93000a; }
