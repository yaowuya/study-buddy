<template>
  <view :class="['plan-card', `plan-${plan.subject || 'default'}`]">
    ...
    <view class="actions">
      <view :class="['plan-action', 'edit-btn', busy ? 'plan-action-disabled' : '']" @tap="!busy && emit('edit', plan.id)">
        <text class="material-symbols-outlined action-icon">edit</text>
        <text>编辑计划</text>
      </view>
      <view :class="['plan-action', 'delete-btn', busy ? 'plan-action-disabled' : '']" @tap="!busy && emit('delete', plan.id)">
