...
def list_tasks(task_date: date | None = None, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _check_family(user)
    tasks = task_crud.get_family_tasks_by_date(db, user.family_id, task_date)
